...
<div class="container">
	...
	<form method="POST" class="form-inline"  action="{#public#}search/name">
		<input type="search"  class="input-medium search-query span3"  id="search"  name="search"  placeholder="举例：北大医学部  董敏"/>
	...
	{if $error|my_count}
   	...
   	<div class="friendInfo thumbnail" >
		...
		<h4><a href="#deleteFriend" data-toggle="modal"  class="delete" data="{$item.user_id}"><button class="btn" >取消收藏</button></a></h4>
   	</div>
   	{/foreach}
   	</div>
							...
							$.getJSON('http://www.days366.com/friend/choosepage?pageNumber='+number,function(data)
									{
										...
										for (var i=0;i<result.result.length;i++)
											...
											var button=$('<h4>').append('<a href="#deleteFriend" data-toggle="modal" class="delete"  data="'+result.result[i].friendId+'"  ><button class="btn ">取消收藏</button></a>');
				...
				$('#deleteFriendButton').live('click',function()
					{
						$.ajax({
							...
							success:function(data)
							{
									...
									{
									var modalheader=$('<div class="modal-header">').append('<a class="close" data-dismiss="modal">&times;</a><h3>取消收藏</h3>');
									var modalbody=$('<div class="modal-body">').append('<p><span class="label label-success" style="font-size:14px">取消收藏成功！</span>');
									var modal=$('<div class="modal fade" id="ajaxError">').append(modalheader)
																													.append(modalbody);
									$('body').append(modal);
									$('#ajaxError').modal('show');
