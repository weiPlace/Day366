...
{	
			...
			case 'jpg':
				{
					...
					$this->getO($img_r);
					if ($this->_o_width<560)
					{
						rename($this->_config->path->data.'/userimg/tmp/'.$this->_imgName, $this->_config->path->data.'/favourite/'.$this->_imgName);
							if (!$this->dealWallPhoto()){return false;}
						return true;
					}
					$scale=$this->_o_width/560;
					...
					}catch(Exception $e)
				{
					...
					$result=imagejpeg($dst_r,$this->_config->path->data.'/favourite/'.$this->_imgName,$jpeg_quality);
			}
			break;
			case 'gif':
			{
					...
					}catch(Exception $e)
				{
					...
					$result=imagegif($dst_r,$this->_config->path->data.'/favourite/'.$this->_imgName,$jpeg_quality);
			}
			break;
			default:
			{
					...
					}catch(Exception $e)
				{
					...
					$result=imagepng($dst_r,$this->_config->path->data.'/favourite/'.$this->_imgName);
			}
		}
   ...
   public function dealWallPhoto()
    {
    	$imgType=explode('.', $this->_imgName);
    	$jpeg_quality=90;
    	$src=$this->_config->path->data.'/favourite/'.$this->_imgName;
    	$this->_width=240;
		switch ($imgType[1])
		{
			case 'jpg':
					...
					}catch(Exception $e)
				{
					...
					$result=imagejpeg($dst_r,$this->_config->path->data.'/wall/'.$this->_imgName,$jpeg_quality);
			}
			break;
			case 'gif':
			{
					...
					}catch(Exception $e)
				{
					...
					$result=imagegif($dst_r,$this->_config->path->data.'/wall/'.$this->_imgName,$jpeg_quality);
			}
			break;
			default:
			{
					...
					}catch(Exception $e)
				{
					...
					$result=imagepng($dst_r,$this->_config->path->data.'/wall/'.$this->_imgName);
			}
		}
		return true;
    }   
