			...
			<div class="span9">
				<div class="result">
					<div class="displayItem row">
						<div class="span302">
					   			<a href="{#public#}friend/findfriendmainpage?id={$content.twitter_user_id}">
					   				<img src="{#imgfinish#}{$content.action_user_img}" alt="This is user image" style="width:80px;height:80px;">
					   			</a>
						</div>
					...
					<h4>{$content.twitter_title}</h4>
		   			...
		   			<p>来源URL:<a href="{$item.twitter_url}">{$content.twitter_url}</a>
		   			{/if}
					   			<div class="buttons">
					   			...
					   			<form class="formComment" onSubmit="return false;" >
					   				...
					   				<button class="btn btn-mini">Comment</button>
					   			</form>
						...
						<h6><a href="http://www.days366.com/friend/findfriendmainpage?id={$item.tc_user_id}">{$item.action_user_name}</a>&nbsp;Comment at{$item.tc_time}</h6>
						<p>{$item.tc_comment}</p>
						</div>
					   	</div>
						{/foreach}
...
$(function(){
	...
	$('.formComment').live('submit',function()
	{
		...
		if (strlen(textContent)>240)
			{
				$('.formComment').append('<label class="error">The character\'s size should not be over 240</label>');
				return false;
		...
		$.ajax(
				{
					...
					success:function(data)
					{
						...
						var imgData=$('<div class="commentImgData pull-left">').append('<a href="#" >'+'<img src="http://www.days366.com/userImg/userimg/small/'+userImg+'" alt="This is image photo" />'+'</a>');
