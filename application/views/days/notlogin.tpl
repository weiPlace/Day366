		...
		{/if}
			<div class="resultImgData">
		   			<a href="{#public#}friend/findfriendmainpage?id={$item.days_user_id}">
		   				<img src="{#imgsmall#}{$item.action_user_image}" alt="This is user image" >
		   			</a>
			</div>
			<div class="resultTextData" data="{$item.days_id}">
		   		<div class="textData">
					...
					<h4>{$item.days_title}</h4>
		   			...
		   			{/if}
		   			<div class="buttons">
						<span class="dayLike"><a class="btn btn-mini" title="Like" class="navItem" href="#accountNotLogin" data-toggle="modal"><i class="icon-heart"></i>&nbsp;<span>{$item.days_like}</span></a></span>
						<span class="dayFavourite"><a class="btn btn-mini" title="Collect" class="navItem" href="#accountNotLogin" data-toggle="modal"><i class="icon-plus-sign"></i>&nbsp;<span>{$item.days_favourite}</span></a></span>
						<span class="dayComment"><a class="btn btn-mini" title="Comment" class="navItem" href="#accountNotLogin" data-toggle="modal"><i class="icon-comment"></i>&nbsp;<span>{$item.days_comment}</span></a></span>
...
$(function(){
	...
	$('#wallNav .add div').popover({
				title:"Tips",
				content:"You can only add something for the day you were born",
				placement:'bottom',
				trigger:'hover'
			});
});
